...
const url = 'https://udndata.com/ndapp/Story?no=7&page=1&udndbid=udndata&SearchString=qcqrSSuk6bTBPj0yMDI1MTAwOCuk6bTBPD0yMDI1MTEwNiuz%2BKdPPcFwpliz%2BA%3D%3D&sharepage=20&select=1&kind=2&article_date=2025-11-01&news_id=10611531'; // 請填入要抓取的網址
...
async function scrapeContent(url: string) {
    ...
    try {
        ...
        const rawCookies = cookieStr.split(';').map(s => s.trim()).filter(Boolean);
        ...
        cdp.Network.responseReceived(async (params: any) => {
            try {
                ...
                if (!resp || !resp.mimeType || !resp.mimeType.startsWith('image/')) return;
                ...
                if (!(pathname.startsWith('/ShowPhoto') || /ShowPhoto/i.test(fullUrl))) return;

                if (captured) return;
                try {
                    const body = await cdp.Network.getResponseBody({ requestId: params.requestId });
                    if (body) {
                        if (body.base64Encoded) {
                            imageBase64 = body.body;
                        } else {
                            imageBase64 = Buffer.from(body.body, 'utf-8').toString('base64');
                        }
                        captured = true;
                    }
                } catch (e) {
    ...
    const storyContent = await driver.wait(until.elementLocated(By.css('.story-content')), 10000);
    // 儲存 story-content 的 outerHTML（含圖片標籤等）
    const storyHtml = await driver.executeScript<string>("return document.querySelector('.story-content') ? document.querySelector('.story-content').outerHTML : null;");
    ...
    // 等待 imageBase64（若有）或直到 timeout
        ...
        // 等待最多 10 秒讓 CDP 收到 image response
        ...
        if (imageBase64) {
            await fs.writeFile('response.jpg', Buffer.from(imageBase64, 'base64'));
        }

        // 保存 story-content 的 HTML（如果有 outerHTML，優先用 outerHTML，否則寫入純文字）
        if (storyHtml) {
            await fs.writeFile('response.html', storyHtml, 'utf-8');
        } else {
            await fs.writeFile('response.html', storyText, 'utf-8');
        }

        // 關閉 CDP 連線
        try { await cdp.close(); } catch (e) { /* ignore */ }
...
scrapeContent(url).then(result => {
    ...
    console.log('Text content saved to response.html');
    if (result.hasImage) {
        console.log('Image saved to response.jpg');
    }
}).catch(error => {
